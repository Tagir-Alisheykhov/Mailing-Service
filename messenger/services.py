...
from messenger.models import Mailing, MailingAttempt, Recipient, Message
...
def send_mailing_attempt():
    """
    Логика попытки отправки почтовой рассылки.
    """
...
def get_mailings_attempt(user=None):
    ...
    if not settings.CACHE_ENABLED:
        ...
        if not unsuccessfully:
            unsuccessfully = MailingAttempt.objects.filter(
                sending_status="unsuccessfully"
            )
